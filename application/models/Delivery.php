...
require_once "base/absDelivery.php";

class Delivery extends absDelivery {
    ...
    public function login(){
        ...
        $this->db->select('clave, idDelivery, name');
        ...
        if($query->num_rows() > 0){
            ...
            if(password_verify($data['clave'], $result['clave'])){
                ...
                // $this->db->where('idDelivery', $result['idDelivery']);
    ...
    public function checkCookie(){
        ...
        $this->db->select('idDelivery, name, code_auth');
        $this->db->from($this->__table);
    ...
    public function redefine(){
        ...
        $this->db->where('idDelivery', $data['idDelivery']);
    ...
    public function remember(){
        ...
        $this->db->select('idDelivery, name');
        $this->db->from($this->__table);
        $this->db->where(['email' => $data['email'],'status' => 1]);
        $this->db->limit(1);
        ...
        if($query->num_rows() > 0){
            ...
            $send = array(
                            ...
                            'content' => site_url() . 'login/redefinir/' . $code.$result['idDelivery'],
                         );
